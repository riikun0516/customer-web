...
- **ユーザー管理・権限**:
  ...
  - 各ユーザーは「アカウント設定」から自分で表示名・メールアドレス・パスワードを変更できます
- **パスワード再設定（メール送信）**: ログイン画面の「パスワードをお忘れですか？」からセルフサービスで再設定可能。自社情報設定でSMTPサーバー（Gmail等）を設定しておく必要があります
...
新しいバージョンをリリースする際は、以下のようにバージョンタグをpushするだけで、`.github/workflows/release.yml` により自動的にGitHub Releaseが作成されます（リリースノートも自動生成されます）。
```bash
git tag v1.1.0
git push origin v1.1.0
```
手動でReleaseを作成したい場合は、GitHubの「Releases」からタグを指定して公開することもできます。

**タグを打つ前に、`web/VERSION` ファイルの中身をタグと同じバージョン番号（例: `1.1.0`。`v`は付けない）に更新してコミットしてください。** このファイルの値は、アプリのサイドバーや「システム更新」画面に表示される「デプロイされているコードのバージョン」として使われます。手動デプロイの場合でもこの表示だけは常に正しいバージョンを示すため、忘れずに更新することをおすすめします。
...
.
├── .github/workflows/
│   └── release.yml       タグpushで自動的にGitHub Releaseを作成
├── VERSION               アプリのバージョン番号（リリース時に更新）
├── config/               DB接続設定（初期セットアップ時に自動生成）
├── includes/             共通処理（DB接続・認証・PDFヘルパー・スキーマ定義・SMTPメール送信）
...
├── forgot_password.php / reset_password.php  パスワード再設定
├── account.php           アカウント設定（表示名・メール・パスワード変更）
...
- ログイン画面に `setup.php`（DB接続設定）へのリンクは置いていません（乗っ取りリスクを避けるため）。再セットアップが必要な場合は `config/config.php` を削除した上で `setup.php` に直接アクセスしてください
- パスワード再設定機能は、ユーザー名の存在有無に関わらず常に同じメッセージを返すことで、ユーザー名の列挙を防いでいます
