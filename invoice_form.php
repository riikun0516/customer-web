...
$invoiceId = isset($_GET['id']) ? (int)$_GET['id'] : (isset($_POST['id']) ? (int)$_POST['id'] : 0);
$isEdit = $invoiceId > 0;
$errors = [];
...
if (!$isEdit && !empty($_GET['case_id'])) {
    ...
    $caseRow = $cStmt->fetch();
    if ($caseRow) {
        $invoice['customer_id'] = $caseRow['customer_id'];
...
if ($isEdit) {
    ...
    $existing = $stmt->fetch();
    if (!$existing) {
        flash_set('error', '指定された請求書が見つかりません');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'save') {
        ...
        $taxAmount = round($subtotal * ($invoice['tax_rate'] / 100), 0);
        ...
        if (!$invoice['customer_id']) {
            ...
            if ($isEdit) {
                $stmt = $pdo->prepare(
                    'UPDATE invoices SET customer_id=?, case_id=?, status=?, issue_date=?, due_date=?, tax_rate=?, subtotal=?, tax_amount=?, total_amount=?, notes=? WHERE id=?'
                );
                $stmt->execute([
                    $invoice['customer_id'], $invoice['case_id'], $invoice['status'], $invoice['issue_date'],
                    $invoice['due_date'], $invoice['tax_rate'], $subtotal, $taxAmount, $total, $invoice['notes'], $invoiceId
                ]);
                $pdo->prepare('DELETE FROM invoice_items WHERE invoice_id=?')->execute([$invoiceId]);
                $targetId = $invoiceId;
            } else {
                $stmt = $pdo->prepare(
                    'INSERT INTO invoices (customer_id, case_id, status, issue_date, due_date, tax_rate, subtotal, tax_amount, total_amount, notes, created_by)
                     VALUES (?,?,?,?,?,?,?,?,?,?,?)'
                );
                $stmt->execute([
                    $invoice['customer_id'], $invoice['case_id'], $invoice['status'], $invoice['issue_date'],
                    $invoice['due_date'], $invoice['tax_rate'], $subtotal, $taxAmount, $total, $invoice['notes'], $me['id']
                ]);
                $targetId = (int)$pdo->lastInsertId();
                $invNumber = 'INV-' . date('Y') . '-' . str_pad($targetId, 4, '0', STR_PAD_LEFT);
                $pdo->prepare('UPDATE invoices SET invoice_number=? WHERE id=?')->execute([$invNumber, $targetId]);
            }

            $itemStmt = $pdo->prepare(
                'INSERT INTO invoice_items (invoice_id, name, quantity, unit_price, amount, sort_order) VALUES (?,?,?,?,?,?)'
            );
            foreach ($items as $idx => $it) {
                $itemStmt->execute([$targetId, $it['name'], $it['quantity'], $it['unit_price'], $it['amount'], $idx]);
            }

            flash_set('success', $isEdit ? '請求書を更新しました' : '請求書を作成しました');
            redirect('invoice_form.php?id=' . $targetId);
        }
    }
}
...
$pageTitle = $isEdit ? '請求書編集' : '新規請求書';
?>

<a href="invoices.php" class="back-link">← 請求書一覧に戻る</a>
<div class="page-header">
  <h2><?= $isEdit ? '請求書編集 ' . e($invoice['invoice_number'] ?? '') : '新規請求書' ?></h2>
  <?php if ($isEdit): ?>
    <a href="invoice_pdf.php?id=<?= (int)$invoiceId ?>" target="_blank" class="btn secondary">PDFを表示</a>
  <?php endif; ?>
</div>
...
<div class="form-card" style="max-width:820px;">
  <form method="post" id="invoiceForm">
    ...
    <?php if ($isEdit): ?><input type="hidden" name="id" value="<?= (int)$invoiceId ?>"><?php endif; ?>
      ...
      <div class="field">
        ...
        <select name="customer_id" required>
          ...
          <?php foreach ($customers as $cu): ?>
            <option value="<?= (int)$cu['id'] ?>" <?= (int)$invoice['customer_id'] === (int)$cu['id'] ? 'selected' : '' ?>><?= e($cu['name']) ?></option>
          <?php endforeach; ?>
        </select>
      </div>
      <div class="field">
        ...
        <select name="case_id" id="caseSelect">
          ...
          <?php foreach ($cases as $c): ?>
            <option value="<?= (int)$c['id'] ?>"
              ...
              <?= (int)($invoice['case_id'] ?? 0) === (int)$c['id'] ? 'selected' : '' ?>><?= e($c['title']) ?></option>
          <?php endforeach; ?>
        </select>
      </div>
    </div>
    ...
    <div class="field-row">
      <div class="field">
        <label>発行日</label>
        <input type="date" name="issue_date" value="<?= e($invoice['issue_date']) ?>">
      </div>
      <div class="field">
        <label>支払期限</label>
    ...
    <div style="margin-top:16px; display:flex; justify-content:flex-end;">
      <table style="width:280px;">
        ...
        <tr>
          <td>消費税
            <input type="number" step="0.01" name="tax_rate" id="taxRateInput" value="<?= e($invoice['tax_rate']) ?>" style="width:60px; display:inline-block; padding:4px;">%
          </td>
    ...
    <div class="form-actions">
      <?php if ($isEdit && is_admin()): ?>
        <button type="submit" name="action" value="delete" class="btn danger" onclick="return confirm('この請求書を削除しますか？');">削除</button>
      <?php endif; ?>
